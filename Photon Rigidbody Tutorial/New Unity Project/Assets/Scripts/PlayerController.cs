...
public class PlayerController : MonoBehaviourPun
{
    ...
    void Update()
    {
        ...
        if (Ball_is_attached == true)
            Ball.transform.position = this.gameObject.transform.localPosition + new Vector3(0, 0.5f, 0) + this.gameObject.transform.forward;
    }
    ...
    public void GrabTheBall()
    {
        if (Ball_is_attached == false && ball.BallIsAttached == false)
        {
            ...
            ball.transform.position = this.gameObject.transform.position + new Vector3(0, 0.5f, 0) + this.gameObject.transform.forward;
